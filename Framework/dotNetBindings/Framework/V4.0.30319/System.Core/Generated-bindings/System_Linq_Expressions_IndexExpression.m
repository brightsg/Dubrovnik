...
//++Dubrovnik.CodeGenerator System_Linq_Expressions_IndexExpression.m
//
...
@implementation System_Linq_Expressions_IndexExpression
...
#pragma mark Properties
    ...
    @synthesize indexer = _indexer;
    - (System_Reflection_PropertyInfo *)indexer
    {
		typedef MonoObject * (*Thunk)(MonoObject *, MonoObject**);
		static Thunk thunk;
		static MonoClass *thunkClass;
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertyGetMethod(thunkClass, "Indexer");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		MonoObject * monoObject = thunk(self.monoObject, &monoException);
		if (monoException != NULL) @throw(NSExceptionFromMonoException(monoException, @{}));
		if ([self object:_indexer isEqualToMonoObject:monoObject]) return _indexer;					
		_indexer = [System_Reflection_PropertyInfo bestObjectWithMonoObject:monoObject];
