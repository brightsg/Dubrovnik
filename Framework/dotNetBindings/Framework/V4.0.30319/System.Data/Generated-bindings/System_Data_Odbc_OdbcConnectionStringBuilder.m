...
//++Dubrovnik.CodeGenerator System_Data_Odbc_OdbcConnectionStringBuilder.m
//
...
@implementation System_Data_Odbc_OdbcConnectionStringBuilder
...
#pragma mark Setup
	...
	{
		return "System.Data.Odbc.OdbcConnectionStringBuilder";
	}
	// obligatory override
...
#pragma mark Constructors
    ...
    + (System_Data_Odbc_OdbcConnectionStringBuilder *)new_withConnectionString:(NSString *)p1
    {
		...
		System_Data_Odbc_OdbcConnectionStringBuilder * object = [[self alloc] initWithSignature:"string" withNumArgs:1, [p1 monoRTInvokeArg]];
...
#pragma mark Properties
    ...
    - (void)setDriver:(NSString *)value
	{
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertySetMethod(thunkClass, "Driver");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		MonoObject *monoException = NULL;
    ...
    - (NSString *)dsn
    {
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertyGetMethod(thunkClass, "Dsn");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		MonoObject * monoObject = thunk(self.monoObject, &monoException);
		if (monoException != NULL) @throw(NSExceptionFromMonoException(monoException, @{}));
		if ([self object:_dsn isEqualToMonoObject:monoObject]) return _dsn;					
		_dsn = [NSString stringWithMonoString:DB_STRING(monoObject)];
    ...
    - (void)setDsn:(NSString *)value
	{
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertySetMethod(thunkClass, "Dsn");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		MonoObject *monoException = NULL;
		thunk(self.monoObject, [value monoObject], &monoException);
