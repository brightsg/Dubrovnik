#import "System_XML.h"
//++Dubrovnik.CodeGenerator System_Xml_Schema_XmlSchemaDocumentation.m
//
...
@implementation System_Xml_Schema_XmlSchemaDocumentation
...
#pragma mark Setup
	...
	{
		return "System.Xml.Schema.XmlSchemaDocumentation";
	}
	// obligatory override
...
#pragma mark Properties
    ...
    - (void)setLanguage:(NSString *)value
	{
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertySetMethod(thunkClass, "Language");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		MonoObject *monoException = NULL;
    ...
    - (DBSystem_Array *)markup
    {
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertyGetMethod(thunkClass, "Markup");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		MonoObject * monoObject = thunk(self.monoObject, &monoException);
		if (monoException != NULL) @throw(NSExceptionFromMonoException(monoException, @{}));
		if ([self object:_markup isEqualToMonoObject:monoObject]) return _markup;					
		_markup = [DBSystem_Array arrayWithMonoArray:DB_ARRAY(monoObject)];

		return _markup;
	}
    - (void)setMarkup:(DBSystem_Array *)value
	{
		_markup = value;
		typedef void (*Thunk)(MonoObject *, MonoObject *, MonoObject**);
		static Thunk thunk;
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertySetMethod(thunkClass, "Markup");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		MonoObject *monoException = NULL;
		thunk(self.monoObject, [value monoObject], &monoException);
