#import "System_XML.h"
//++Dubrovnik.CodeGenerator System_Xml_Serialization_XmlSerializerVersionAttribute.m
//
...
@implementation System_Xml_Serialization_XmlSerializerVersionAttribute
...
#pragma mark Constructors
    ...
    + (System_Xml_Serialization_XmlSerializerVersionAttribute *)new_withType:(System_Type *)p1
    {
		...
		System_Xml_Serialization_XmlSerializerVersionAttribute * object = [[self alloc] initWithSignature:"System.Type" withNumArgs:1, [p1 monoRTInvokeArg]];
...
#pragma mark Properties
	...
	// Managed property type : System.String
    @synthesize parentAssemblyId = _parentAssemblyId;
    - (NSString *)parentAssemblyId
    {
		typedef MonoObject * (*Thunk)(MonoObject *, MonoObject**);
		static Thunk thunk;
		static MonoClass *thunkClass;
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertyGetMethod(thunkClass, "ParentAssemblyId");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		MonoObject * monoObject = thunk(self.monoObject, &monoException);
		if (monoException != NULL) @throw(NSExceptionFromMonoException(monoException, @{}));
		if ([self object:_parentAssemblyId isEqualToMonoObject:monoObject]) return _parentAssemblyId;					
		_parentAssemblyId = [NSString stringWithMonoString:DB_STRING(monoObject)];

		return _parentAssemblyId;
	}
    - (void)setParentAssemblyId:(NSString *)value
	{
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertySetMethod(thunkClass, "ParentAssemblyId");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		MonoObject *monoException = NULL;
		thunk(self.monoObject, [value monoObject], &monoException);
