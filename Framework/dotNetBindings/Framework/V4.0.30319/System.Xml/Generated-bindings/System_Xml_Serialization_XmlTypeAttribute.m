#import "System_XML.h"
//++Dubrovnik.CodeGenerator System_Xml_Serialization_XmlTypeAttribute.m
//
...
@implementation System_Xml_Serialization_XmlTypeAttribute
...
#pragma mark Constructors
    ...
    + (System_Xml_Serialization_XmlTypeAttribute *)new_withTypeName:(NSString *)p1
    {
		...
		System_Xml_Serialization_XmlTypeAttribute * object = [[self alloc] initWithSignature:"string" withNumArgs:1, [p1 monoRTInvokeArg]];
...
#pragma mark Properties
    ...
    @synthesize anonymousType = _anonymousType;
    - (BOOL)anonymousType
    {
		...
		if (!thunk || thunkClass != self.monoClass) {
			...
			MonoMethod *monoMethod = GetPropertyGetMethod(thunkClass, "AnonymousType");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		BOOL monoObject = thunk(self.monoObject, &monoException);
		if (monoException != NULL) @throw(NSExceptionFromMonoException(monoException, @{}));
		_anonymousType = monoObject;

		return _anonymousType;
	}
    - (void)setAnonymousType:(BOOL)value
	{
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertySetMethod(thunkClass, "AnonymousType");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		MonoObject *monoException = NULL;
