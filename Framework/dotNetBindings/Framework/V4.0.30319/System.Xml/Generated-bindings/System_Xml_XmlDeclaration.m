#import "System_XML.h"
//++Dubrovnik.CodeGenerator System_Xml_XmlDeclaration.m
//
...
@implementation System_Xml_XmlDeclaration
...
#pragma mark Properties
    ...
    - (NSString *)encoding
    {
		...
		_encoding = [NSString stringWithMonoString:DB_STRING(monoObject)];

		return _encoding;
	}
    - (void)setEncoding:(NSString *)value
	{
		_encoding = value;
    ...
    - (NSString *)standalone
    {
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertyGetMethod(thunkClass, "Standalone");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		MonoObject * monoObject = thunk(self.monoObject, &monoException);
		if (monoException != NULL) @throw(NSExceptionFromMonoException(monoException, @{}));
		if ([self object:_standalone isEqualToMonoObject:monoObject]) return _standalone;					
		_standalone = [NSString stringWithMonoString:DB_STRING(monoObject)];

		return _standalone;
	}
    - (void)setStandalone:(NSString *)value
	{
		_standalone = value;
		typedef void (*Thunk)(MonoObject *, MonoObject *, MonoObject**);
		static Thunk thunk;
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertySetMethod(thunkClass, "Standalone");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		MonoObject *monoException = NULL;
		thunk(self.monoObject, [value monoObject], &monoException);
