#import "System_XML.h"
//++Dubrovnik.CodeGenerator System_Xml_XmlEntity.m
//
...
@implementation System_Xml_XmlEntity
...
#pragma mark Properties
    ...
    - (NSString *)notationName
    {
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertyGetMethod(thunkClass, "NotationName");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		MonoObject * monoObject = thunk(self.monoObject, &monoException);
		if (monoException != NULL) @throw(NSExceptionFromMonoException(monoException, @{}));
		if ([self object:_notationName isEqualToMonoObject:monoObject]) return _notationName;					
		_notationName = [NSString stringWithMonoString:DB_STRING(monoObject)];
