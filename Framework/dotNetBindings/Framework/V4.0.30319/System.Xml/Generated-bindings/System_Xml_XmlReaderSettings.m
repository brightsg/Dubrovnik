...
#pragma mark Constructors
    ...
    + (System_Xml_XmlReaderSettings *)new_withResolver:(System_Xml_XmlResolver *)p1
    {
		...
		System_Xml_XmlReaderSettings * object = [[self alloc] initWithSignature:"System.Xml.XmlResolver" withNumArgs:1, [p1 monoRTInvokeArg]];
...
#pragma mark Properties
    ...
    @synthesize validationFlags = _validationFlags;
    - (int32_t)validationFlags
    {
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertyGetMethod(thunkClass, "ValidationFlags");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		int32_t monoObject = thunk(self.monoObject, &monoException);
		if (monoException != NULL) @throw(NSExceptionFromMonoException(monoException, @{}));
		_validationFlags = monoObject;

		return _validationFlags;
	}
    - (void)setValidationFlags:(int32_t)value
	{
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertySetMethod(thunkClass, "ValidationFlags");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		MonoObject *monoException = NULL;
		thunk(self.monoObject, value, &monoException);
