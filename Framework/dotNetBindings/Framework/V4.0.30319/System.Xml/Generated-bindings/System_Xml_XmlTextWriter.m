...
#pragma mark Constructors
    ...
    + (System_Xml_XmlTextWriter *)new_withW:(System_IO_TextWriter *)p1
    {
		...
		System_Xml_XmlTextWriter * object = [[self alloc] initWithSignature:"System.IO.TextWriter" withNumArgs:1, [p1 monoRTInvokeArg]];
...
#pragma mark Properties
    ...
    @synthesize formatting = _formatting;
    - (int32_t)formatting
    {
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertyGetMethod(thunkClass, "Formatting");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		int32_t monoObject = thunk(self.monoObject, &monoException);
		if (monoException != NULL) @throw(NSExceptionFromMonoException(monoException, @{}));
		_formatting = monoObject;

		return _formatting;
	}
    - (void)setFormatting:(int32_t)value
	{
		_formatting = value;
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertySetMethod(thunkClass, "Formatting");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		MonoObject *monoException = NULL;
