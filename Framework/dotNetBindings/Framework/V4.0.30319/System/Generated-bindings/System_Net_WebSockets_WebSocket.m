...
#pragma mark Properties
    ...
    + (System_TimeSpan *)defaultKeepAliveInterval
    {
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertyGetMethod(thunkClass, "DefaultKeepAliveInterval");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
...
#pragma mark Methods
    ...
    + (System_Net_WebSockets_WebSocket *)createClientWebSocket_withInnerStream:(System_IO_Stream *)p1 subProtocol:(NSString *)p2 receiveBufferSize:(int32_t)p3 sendBufferSize:(int32_t)p4 keepAliveInterval:(System_TimeSpan *)p5 useZeroMaskingKey:(BOOL)p6 internalBuffer:(System_ArraySegmentA1 *)p7
    {
		...
		MonoObject *monoObject = [self invokeMonoClassMethod:"CreateClientWebSocket(System.IO.Stream,string,int,int,System.TimeSpan,bool,System.ArraySegment`1<byte>)" withNumArgs:7, [p1 monoRTInvokeArg], [p2 monoRTInvokeArg], DB_VALUE(p3), DB_VALUE(p4), [p5 monoRTInvokeArg], DB_VALUE(p6), [p7 monoRTInvokeArg]];
