#import "mscorlib.h"
//++Dubrovnik.CodeGenerator System_Globalization_HijriCalendar.m
//
...
#pragma mark Fields
	...
	// Managed field name : HijriEra
	// Managed field type : System.Int32
    static int32_t m_hijriEra;
    + (int32_t)hijriEra
    {
		MonoObject *monoObject = [[self class] getMonoClassField:"HijriEra"];
		m_hijriEra = DB_UNBOX_INT32(monoObject);
...
#pragma mark Properties
	...
	// Managed property name : HijriAdjustment
	// Managed property type : System.Int32
    @synthesize hijriAdjustment = _hijriAdjustment;
    - (int32_t)hijriAdjustment
    {
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertyGetMethod(thunkClass, "HijriAdjustment");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		int32_t monoObject = thunk(self.monoObject, &monoException);
		if (monoException != NULL) @throw(NSExceptionFromMonoException(monoException, @{}));
		_hijriAdjustment = monoObject;
    ...
    - (void)setHijriAdjustment:(int32_t)value
	{
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertySetMethod(thunkClass, "HijriAdjustment");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		MonoObject *monoException = NULL;
