...
#pragma mark Properties
    ...
    - (float)exIntTestProperty
    {
		...
		if (!thunk || thunkClass != self.monoClass) {
			...
			MonoMethod *monoMethod = GetPropertyGetMethod(thunkClass, "Dubrovnik.UnitTests.IReferenceObject2.ExIntTestProperty");
    ...
    - (void)setExIntTestProperty:(float)value
	{
		...
		if (!thunk || thunkClass != self.monoClass) {
			thunkClass = self.monoClass;
			MonoMethod *monoMethod = GetPropertySetMethod(thunkClass, "Dubrovnik.UnitTests.IReferenceObject2.ExIntTestProperty");
			thunk = (Thunk)mono_method_get_unmanaged_thunk(monoMethod);
		}
		MonoObject *monoException = NULL;
